...
/**
 * Created by Johannes on 26.10.2017. For complex logging.
 * TODO: This may be replaced by commons csv.
 */
public class CSVSink {
    ...
    public CSVSink(String path, Charset charset, SinkType type, String... staticHeader) {
        ...
        switch (type) {
            case STATIC: {
                for (String x : staticHeader) header.add(x);
    ...
    private void initializeWriter() {
        try {
            writer = CSVFormat.DEFAULT.print(new File(path), charset);//new CSVWriter(new FileWriter(new File(path)), ',', CSVWriter.NO_QUOTE_CHARACTER);
        } catch (IOException e) {
        ...
        switch (type) {
            ...
            case HEADER_FILE: {
                if (header.addAll(row.keySet())) {
                    try {
                        Files.asCharSink(new File(path.substring(0, path.lastIndexOf(".")) + ".header"), charset).write(Joiner.on(",").join(header));
                    } catch (IOException e) {
...
//    public CSVSink(String filePath, Charset charset, boolean separateHeader, String... header) {
//        this.charset = charset;
//        csvFile = new File(filePath);
//        headerFile = separateHeader ? new File(filePath.substring(0, filePath.lastIndexOf(".")) + ".header") : null;
//
